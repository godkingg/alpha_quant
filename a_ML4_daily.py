# ============================================================
# a_ML2_pure.py — PURE ML RECURSIVE (RF + ZLEMA5 + Core+Momentum DELTA)
# 
# Khác biệt với a_ML2.py:
#   - KHÔNG blend với trend/ZLEMA
#   - PURE ML prediction: 100% từ Random Forest
#   - Recursive forecast: synthetic data → recompute features → predict
#   - Không có noise injection, không có trend extrapolation
# 
...
def predict_next_price(past_window: pd.DataFrame,
    ...
    """
    PURE ML prediction: close[t+1] = zlema[t] + predicted_residual
    Không blend, không noise
    """
    if lookback is None:
        lookback = CONFIG["lookback"]
    ...
    # PURE ML: không blend, chỉ dùng ML prediction
...
def get_model_paths(symbol: str) -> dict:
    s = symbol.upper()
    return {
        "model"         : os.path.join(SAVE_DIR, f"rf_pure_z5_{s}.joblib"),
        "f_scaler"      : os.path.join(SAVE_DIR, f"f_scaler_pure_{s}.joblib"),
        "t_scaler"      : os.path.join(SAVE_DIR, f"t_scaler_pure_{s}.joblib"),
        "meta"          : os.path.join(SAVE_DIR, f"meta_pure_{s}.json"),
    }
...
def save_model_package(symbol: str, model, f_scaler, t_scaler,
    ...
    print(f"✅ Saved PURE RF model for {s}")
...
def train_model_for_symbol(symbol: str) -> tuple:
    s = symbol.upper()
    print(f"\n{'='*60}")
    print(f"TRAIN (PURE ML)  {s}  |  RF + ZLEMA({CONFIG['zlema_period']})")
    print(f"{'='*60}")
...
# ============================================================
# FORECAST — PURE ML RECURSIVE (NO HYBRID)
# ============================================================

def forecast_future(symbol: str,
                    forecast_steps: int = 5) -> pd.DataFrame:
    """
    ✅ PURE ML RECURSIVE FORECAST
    
    Strategy:
    1. Predict với ML (100%)
    2. Append synthetic row
    3. Recompute features
    4. Repeat
    
    NO:
    - Trend extrapolation
    - ZLEMA anchor blending
    - Noise injection
    ...
    print(f"\n{'='*60}")
    print(f"PURE ML FORECAST {forecast_steps} PHIÊN  |  {s}")
...
def backtest_model(symbol: str,
    ...
    print(f"\n{'='*60}")
    print(f"BACKTEST (PURE ML)  {s}  từ {backtest_start}")
    print(f"RF + ZLEMA({CONFIG['zlema_period']}) | NO HYBRID")
    print(f"{'='*60}")
...
def main():
    print("\n" + "="*60)
    print("PURE ML RECURSIVE FORECAST")
    ...
    while True:
        ...
        print("1. Dự báo giá (PURE ML recursive)")
        print("2. Backtest (PURE ML)")
