# ============================================================
# a_dashboard_v5.py — Dashboard terminal-style v5
# Kết hợp với a_ML5.py (Pure RF + ZLEMA5 + Core+Momentum DELTA)
# PURE ML — không hybrid, không trend extrapolation
# ============================================================
...
from a_ML4_daily import (model_exists, train_model_for_symbol,
...
def html_to_image4(html_file: str, png_file: str = None,
                  width: int = 1600) -> str:
    if png_file is None:
...
def get_data_with_indicators_and_zlema(symbol: str,
    ...
    # ZLEMA period đồng bộ với a_ML5
    ...
    # Slope(5) theo ZLEMA — dùng trong feature ML
    z_ser = df["zlema_val"]
    df["slope_5"] = ((z_ser - z_ser.shift(5)) / z_ser.shift(5)) * 100
...
def generate_signals(latest: pd.Series) -> dict:
    """Giữ lại cho score engine và card Xu hướng."""
    s = {}
    rsi = latest["rsi"]
...
def build_main_chart_fragment(df: pd.DataFrame,
    ...
    if n_fc > 0:
        ...
        fc_line = (
            Line()
            .add_xaxis(all_dates)
            .add_yaxis("🔮 Dự báo (Pure RF)", pred_ser,
...
def create_terminal_dashboard4(symbol: str,
                               months: int = 6,
                               forecast_steps: int = 5) -> str:
    symbol = symbol.upper()

    # Ensure model (auto-retrain nếu cũ — dùng a_ML5)
    ensure_model(symbol)
    ...
    zlema_period = ML_CONFIG.get("zlema_period", 5)
    retrain_every = ML_CONFIG.get("retrain_every", 5)
...
<title>{symbol} Dashboard — Pure RF v5</title>
...
.g-main{{display:flex;gap:14px;margin-bottom:14px;align-items:stretch}}
...
.pure-rf-tag{{
  ...
  color:#00e5ff;border:1px solid #00e5ff44;background:rgba(0,229,255,.06);
...
<div class="hdr">
  ...
  <span class="hdr-sep">│</span>
  <span class="pure-rf-tag">PURE RF</span>
  <span class="hdr-badge">ZLEMA({zlema_period}) · Random Forest · NO HYBRID</span>
</div>
...
<div class="g-main">
  <div class="card" style="flex:2;padding:8px;min-width:0">
    <div style="
        ...
        color:#e6e6e6;margin:14px 0 12px 0;letter-spacing:.5px">
      📈 {symbol} — BIỂU ĐỒ GIÁ &amp; DỰ BÁO
      <span class="pure-rf-tag" style="margin-left:8px;font-size:12px">PURE RF</span>
    </div>
    {chart_html}
  </div>
  ...
  <div class="card" style="flex:1;display:flex;flex-direction:column;min-width:320px">
    <div class="stitle" style="font-size:16px">
      ...
      <span class="pure-rf-tag" style="margin-left:6px">Pure RF</span>
    </div>
    ...
    <div style="margin-top:12px;font-size:11px;color:#484f58;text-align:center;
      ...
      * Khoảng tin cậy ≈ Giá ± 0.5 × ATR ({latest_atr:,.2f})<br/>
      * Model: Random Forest — NO HYBRID
    </div>
  </div>
</div>
...
<div class="foot">
  ...
  Model: <b>Pure Random Forest</b> + ZLEMA({zlema_period}) &nbsp;•&nbsp;
  NO Hybrid · NO Trend · NO Noise &nbsp;•&nbsp;
  Retrain mỗi {retrain_every} ngày &nbsp;•&nbsp;
  <span style="color:#30363d">Dữ liệu chỉ mang tính tham khảo, không phải khuyến nghị đầu tư</span>
</div>
...
setTimeout(function() {{ window.dispatchEvent(new Event('resize')); }}, 200);
    ...
    fname = f"terminal_dashboard_v5_{symbol}_{ts}.html"
    with open(fname, "w", encoding="utf-8") as f:
        f.write(html)

    print(f"✅ Dashboard v5 (Pure RF): {fname}")
    return fname
...
if __name__ == "__main__":
    ...
    fname  = create_terminal_dashboard4(symbol, months=6, forecast_steps=5)
    # Uncomment dòng dưới để xuất ảnh PNG:
    # html_to_image(fname)